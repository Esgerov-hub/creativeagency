                                        ...
                                        <div class="tab-pane @if(++$key ==1) active @endif" id="{{$lang['code']}}"
                                            ...
                                            <div class="row g-3">
                                                <div class="col-12">
                                                    ...
                                                    <input type="text" class="form-control" name="title[{{$lang['code']}}]" value="{{ !empty($laboratory['title'][$lang['code']])? $laboratory['title'][$lang['code']]: NULL }}">
                                                </div>
                                                <div class="col-12">
                                                    <label class="form-label">@lang('admin.text') - {{$lang['code']}}</label>
                                                    <textarea class="form-control" type="text" name="text[{{$lang['code']}}]" >{{ !empty($laboratory['text'][$lang['code']])? $laboratory['text'][$lang['code']]: NULL }}</textarea>
                                                </div>
                                                <div class="col-12">
                                                    ...
                                                    <textarea class="editor form-control"
                                                              ...
                                                              name="fulltext[{{$lang['code']}}]">{{ !empty($laboratory['fulltext'][$lang['code']])? $laboratory['fulltext'][$lang['code']]: NULL }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <select class="form-control" name="category_id" id="input-category">
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{$category->id}}" @if($category->id ==  $laboratory->category_id) selected @endif>{{ !empty(json_decode($category, true)['title'][$currentLang])? json_decode($category, true)['title'][$currentLang]: null }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-12">
                                            <label for="city_id" class="form-label">@lang('admin.cities')</label>
                                            <select class="form-control" name="city_id" id="city_id">
                                                <option value="">Sec</option>
                                                @foreach($cities as $city)
                                                    <option value="{{$city->id}}" @if($city->id ==  $laboratory->city_id) selected @endif>{{ !empty(json_decode($city, true)['name'][$currentLang])? json_decode($city, true)['name'][$currentLang]: null }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-control" name="status">
                                                <option value="1" @if($laboratory['status'] ==1) selected @endif>@lang('admin.active')</option>
                                                <option value="0" @if($laboratory['status'] ==0) selected @endif>@lang('admin.nonactive')</option>
                                            </select>
                                        </div>

                                        <div class="col-md-4">
                                            <label class="form-label">@lang('admin.datetime')</label>
                                            <input type="datetime-local" class="form-control" name="datetime" value="{{ date('Y-m-d\TH:i', strtotime($laboratory->datetime)) }}">
                                        </div>
                                        <div class="col-md-4">
                                            <label class="form-label">@lang('admin.is_main')</label>
                                            <select class="form-control" name="is_main">
                                                <option value="1" @if($laboratory['is_main'] ==1) selected @endif>@lang('admin.show')</option>
                                                <option value="0" @if($laboratory['is_main'] ==0) selected @endif>@lang('admin.nonshow')</option>
                                            </select>
                                        </div>
                                        <div class="col-sm-12">
                                                    ...
                                                    <div class="card-body">
                                                        <div class="row">
                                                            <div class="col-xxl-9 col-sm-8">
                                                                ...
                                                                <div class="col-md-5">
                                                                    <img src="{{ asset('uploads/laboratory/'.$laboratory->image) }}" style="width: 288px;!important;">
                                                                </div>
                                                            </div>
                                                ...
                                                <div class="slider-images" style="display: flex; flex-wrap: wrap; gap: 10px;"> <!-- Flex container -->
                                                    @foreach($laboratory['slider_image'] as $slider_image)
                                                        <div style="flex: 0 0 30%;"> <!-- Her resmin genişliği %30 -->
                                                            <img src="{{ asset('uploads/laboratory/slider_image/'.$slider_image) }}" style="width:100%; height: auto; padding: 10px!important;">
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
