...
@section('admin.content')
    ...
    <div class="main-content">
        <div class="dashboard-breadcrumb mb-25">
            <h2>@lang('admin.laboratory')</h2>
            <a class="btn btn-sm btn-primary" href="{{ route('admin.laboratory.create') }}">
            ...
            <div class="col-12">
                <div class="panel">
                    <div class="panel-body">
                        <div class="row">
                            @if(!empty($laboratories[0]) && isset($laboratories[0]))
                                @foreach($laboratories as $data)
                                    <div class="col-sm-6">
                                        <div class="card">
                                            <div class="card-header">
                                                {{ !empty($data['title'][$currentLang])? $data['title'][$currentLang]: null }}
                                                <a href="{{ route('admin.laboratory.edit',$data['id']) }}" class="btn btn-sm btn-icon btn-primary" title="@lang('admin.edit')">
                                            ...
                                            <div class="card-body animation-card">
                                                <div class="text-center" data-aos="flip-left">
                                                    <img src="{{ asset('uploads/laboratory/'.$data->image) }}" alt="{{ !empty($data['title']['az'])? $data['title']['az']: $data['title']['az'] }}">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="delete{{$value['id']}}" tabindex="-1" aria-labelledby="deletecategory{{$value['id']}}Label" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.laboratory.destroy',$value['id']) }}" method="POST">
                            @csrf
                            @method('DELETE')
